    ...
    useEffect(() => {
        const handleBeforeUnload = async (e) => {
          ...
          // Update the scanner state in the database
            await axios.post('http://localhost:8000/scanner/scanner-state-false',
                JSON.stringify({
        ...
        try{
            if(productLists.length ==0){
                setErrMsg('No item in the cart. Please add one');
                    ...
                    const response = await productLists.map( async(element) => await axios.post('http://localhost:8000/sale/addsale',
                JSON.stringify({
                    "card_number": 181003,
                    "product_info": {
